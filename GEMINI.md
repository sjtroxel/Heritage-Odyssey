...
- **Narrative Display:** Render narratives using a high-contrast modal overlay (`z-[60]`) with a blurred background. Use split-and-map on double newlines (`\n\n+`) for consistent paragraph styling.
- **Voice/Audio Control:** Integrate Pause/Resume controls directly into the narrative modal, linked to the `useNarrativePipeline` playback state.
