...
const InteractionLayer: React.FC<InteractionLayerProps> = ({
  ...
  return (
    <>
      ...
      {/* Narrative modal — portaled to <body>. It must escape BOTH the panel
          (which has a permanent `transform` for the minimize slide, plus
          `overflow-hidden`) AND the Framer `motion.div` scene wrapper in App.tsx.
          Any transformed ancestor becomes the containing block for a
          `position: fixed` child and would clip it; a portal to document.body is
          the only robust fix. Do NOT inline this back into the tree. */}
      {isNarrativeOpen &&
        narrativeText &&
        createPortal(
          <div
