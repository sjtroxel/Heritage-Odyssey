...
1. **Four technical blog post series — IN PROGRESS as of 2026-05-09.** Macro plan locked in `BLOG_SERIES_PLAN_MAY_2026.md`. Order (by increasing complexity): ChronoQuizzr → Poster Pilot → Asteroid Bonanza → Wildlife Sentinel. Cadence: ~2–3 days between posts. Total window: ~10 days. **Post #1 (ChronoQuizzr) is drafted and ready** at `prior-project-blog-posts/CHRONO_QUIZZR_BLOG.md` — user is deciding the publish date. Asteroid Bonanza rate-limit work is the only known blocker, and only blocks post #3 specifically (must ship in a separate Claude session before post #3 publishes; ~30–60 min of work).
