...
Series-execution day 1. The "four technical blog posts" item from the May 8 pending list transitioned from idea to active workstream. Macro plan locked in, post #1 (ChronoQuizzr) drafted in full, video reviewed and adopted. User has not yet decided whether to publish post #1 today or hold for a stronger publish window.

- **Macro plan written.** `next-project-planning/BLOG_SERIES_PLAN_MAY_2026.md` is now the source of truth for the four-post series. Locks in: order (ChronoQuizzr → Poster Pilot → Asteroid Bonanza → Wildlife Sentinel by *increasing* complexity — note this reverses the order listed in the May 8 pending entry), cadence (~2–3 days between posts, total window ~10 days), length (~200 words for #1–2, scaling to 350–400 for #3–4), per-post template, hashtag set, AI-tells to strip, archive convention (`prior-project-blog-posts/`), and the pre-post-#3 Asteroid Bonanza rate-limit dependency.

- **Archive folder created.** `next-project-planning/prior-project-blog-posts/` mirrors the existing `prior-project-readme-docs/` naming convention. Each post's source markdown saves there as `PROJECT_NAME_BLOG.md` so the user owns the durable artifact regardless of LinkedIn's in-feed decay.

- **Post #1 (ChronoQuizzr) drafted in full.** Saved at `prior-project-blog-posts/CHRONO_QUIZZR_BLOG.md`. ~210 words / ~1,330 chars (well under LinkedIn's 3,000-char cap). Hashtags: `#AIEngineering #LLMs #TypeScript #OpenToWork`. Lead technical angle is the Generate → Attack → Rewrite adversarial verification loop in the Chronicler Engine — chosen because two-agent verification is the most signal-rich detail in the codebase for a recruiter who can read code; "I built a quiz game" would have buried that. Hook line ("Built a historical-events GeoGuessr where Claude writes each clue and a second Claude tries to break it") sits at ~110 chars, well inside the desktop "...see more" cut-point of ~210 chars.

- **Visual: still image plan replaced with native video.** Original plan was a single hero screenshot of mid-round gameplay. User produced a 42s silent screencast instead (`Recording 2026-05-09 094327.mp4`, 24 MB, full play loop on a 1791 Haitian Revolution clue). After frame-by-frame review: the video was kept as-is. Rationale: the climax frame at ~32–33s shows both pins on Hispaniola with the parchment Aged Map theme, "4,696 pts / 125.5 km from the true location," and the reveal "Bois Caïman, Cap-Français, Haiti" — proximity-but-not-perfect tells the product story far better than a single screenshot could. The clip also bonus-demonstrates the dual-theme system (Inky Night during gameplay → Aged Map during reveal) which the original plan had not contemplated. Native video on the 2026 LinkedIn feed also gets stronger algorithmic lift than stills. Decision: post #1 uploads as native video. Same pattern likely applies to later posts where the user can produce a clean loop clip.

- **Tooling: `ffmpeg` installed system-wide.** Required (`sudo apt install ffmpeg`) so Claude can extract video frames for review without manual screenshotting. Future sessions can use it directly. Standard frame-extract command: `ffmpeg -ss <seconds> -i <video.mp4> -frames:v 1 <output>.png -loglevel error`.

- **First-post anxiety addressed.** User's instinct, on first ever LinkedIn blog post, was to lead with a soft-launch meta intro ("I'm going to be making blog posts about all the apps I've made..."). Pushed back: the first 1–2 lines of a feed post are the highest-leverage real estate, and meta-content gets scrolled past. The verb-led "Built X..." opener is a deliberate LinkedIn pattern, not a tonal accident. Offered an alternative — a separate, very short standalone "starting a series this week" announcement post the day before — if user wants the announcement frame. User accepted the verb-led opener for post #1; the optional series-announcement post was not adopted.

- **Side discussion: Codefi instructor's original message.** User shared the original Codefi-instructor message that catalysed this whole career-positioning push (the "Real talk... you're overthinking this... build something real and put it out there" message from a couple weeks back) and asked whether the instructor had used AI to write it. Assessed as written by a human: responsive to specifics the user had told the instructor; uneven sentence rhythm with afterthought-style postscripts; no em dashes; no "I'm thrilled to share" template tics. The instructor's recommended sequence ("Build → Document and post publicly → Rinse and repeat") is exactly what the macro plan executes on. One instructor-recommended practice not yet built into the macro plan: "Comment meaningfully on 2–3 posts from people doing AI work." Worth folding in as a side practice during the series window.

### May 11, 2026

Series-execution day 2. Three workstreams executed in one ~3-hour morning session: post #1 published; LinkedIn privacy/visibility audit done with one significant finding; post #2 fully drafted with native video produced and awaiting publish. Detailed log of all three follows.

#### A. ChronoQuizzr post #1 — PUBLISHED

- **Published to LinkedIn ~mid-morning 2026-05-11** (Monday). The original plan recommended Tue/Wed/Thu as the absolute-peak slot, but Monday morning is the next-best window and the cost of waiting another 24h (decision fatigue, lost momentum) outweighed the marginal off-peak penalty. User posted the locked draft from `prior-project-blog-posts/CHRONO_QUIZZR_BLOG.md` as native video (the 42s screencast from 2026-05-09).
- **Early metrics:** 10 impressions in the first 5 minutes. Discussion noted this is normal "algorithmic test boost" territory — the first 30–90 minutes is the distribution-decision window where LinkedIn samples a small slice of the network and watches dwell time, engagement, and click-through. Real verdict comes at the 1–6 hour mark, not at 5 minutes. User cautioned against refreshing analytics obsessively.
- **No comment-reply opportunities** surfaced during the session window. If any comments land, user knows to reply within the first hour for algorithmic boost.

#### B. LinkedIn privacy / visibility audit

User asked whether recruiter inbound is realistic and requested a full settings audit. Two-layer framework used: **discoverability** (can recruiters find you) and **reachability** (can they contact you when they do). One major finding plus six smaller adjustments.

- **MAJOR FINDING: Public profile visibility was OFF.** Discovered while walking through `Settings → Visibility → Public profile settings`. This meant: profile was invisible to Google search, invisible to anyone not logged in to LinkedIn, broken on profile-badge embeds, and broken in third-party recruiter scraping tools. The ChronoQuizzr post itself was unaffected (feed distribution to logged-in users is a separate system, confirmed by the 10-impression count), but the audit was triggered by the right impulse — the post would have under-performed its potential ceiling otherwise. **Flipped on** with all profile sections (photo, headline, summary, experience, education, skills, Featured, activity) toggled to public-visible. User verified the fix by viewing `linkedin.com/in/sjtroxel` in an incognito window.

- **Six smaller settings changes:**
  - `Discoverability by email`: 1st/2nd degree → **Everyone** (this is search-by-email, not display — does not expose the email anywhere; recruiters who already have the email from GitHub/portfolio can find the LinkedIn).
  - `Discoverability by phone number`: 2nd-degree → **Everyone** (same logic).
  - `Share profile updates`: Off → **On** (distribution surface for blog-series-window profile changes).
  - `Off-LinkedIn visibility`: Off → **On** (third-party recruiter tools can pull approved profile data).
  - `Last name visibility`: confirmed **full last name** (no "Steffan T." privacy initial).
  - `Email visibility` (display): set to **1st + 2nd degree connections**. One click looser than the recommendation of 1st-degree-only, deliberate user call — 2nd-degree on LinkedIn is mostly Codefi-cohort and developer network for this user. Defensible trade-off; small scraping risk accepted.

- **One deferred decision:** `Visibility when viewing other profiles` — currently `Private mode`. The recommendation was the middle option (`Private profile characteristics`), which preserves the recruiter reverse-view warm-lead channel without naming the user to people they know personally. User found the surveillance feel uncomfortable ("ew") and explicitly asked to be re-prompted later. Saved to memory (`project_linkedin_privacy_audit_2026_05_11.md`) so a future session re-raises gently, not pushy.

- **Mid-audit conceptual conflation, corrected:** "Discoverability by email/phone" and "Email/phone visibility" sound similar but do different things (search vs display). User correctly objected when they were conflated mid-conversation. Future audits should name the distinction explicitly upfront. Captured in the audit memory file.

- **The privacy audit and the ChronoQuizzr publish overlapped by ~30 minutes** — the public-visibility-OFF setting was active for the first ~30 min of the post being live. Discussion concluded this had near-zero real cost: feed distribution to logged-in users worked fine (proven by the 10-impression count); the only theoretical loss was logged-out Google-search clicks during that window, which is unrealistic given Google's re-crawl interval. User considered deleting and reposting; pushed back firmly — delete-and-repost triggers LinkedIn's duplicate-content penalty and resets the algorithmic test window. Net call: keep the post live, fix the setting forward, accept the trivial early-window loss. User accepted.

#### C. Poster Pilot post #2 — FULLY DRAFTED with native video

The harder post in the series, anticipated as such. Multiple strategic decisions captured below.

- **Lead-angle strategy.** Three candidate leads considered: (1) the Red Button human-handoff feature, (2) vibe mode + Reciprocal Rank Fusion, (3) the 4-mode multimodal search overview. Initial recommendation was the Red Button — same "judgment about LLM failure modes" pattern as the Adversary loop in ChronoQuizzr. **User correctly rejected this** on a sharp observation: ChronoQuizzr's Adversary loop *prevents* failure; the Red Button *accepts* failure. Poster Pilot's value prop is to replace the friction of contacting a human archivist — leading with "the app admits when it can't help" foregrounds exactly the outcome the user is trying to avoid. Not parallel. Pivoted to vibe + RRF as the lead. Red Button moved to one bullet mid-list as honest engineering framing, not the hook.

- **"Not a wrapper around cosine similarity" is the signal sentence.** Reciprocal Rank Fusion is a real IR technique from TREC research; name-checking it separates the post from the LangChain-tutorial layer. This sentence stays in any future edit of the post.

- **5,000-poster honesty paragraph included** per §10 of the blog-series macro plan. Converts the dataset's long-tail gap ("pig problem") into evidence of production-scale awareness rather than a hidden weakness.

- **"Don't lead with Claude" — new feedback principle adopted.** User flagged mid-draft that two-of-two posts leading with "Claude" risks narrowing recruiter perception to "Claude-specifically engineer" rather than "AI engineer." Important specifically because a meaningful portion of the user's TAM (companies using OpenAI / Bedrock / Llama / Gemini) could read the feed and self-disqualify. Saved as `feedback_dont_lead_with_claude.md`. Applied: the hook no longer leads with "Claude"; the model is named in the body where it carries technical signal. Will be applied to posts #3 and #4 too (Asteroid swarm post should lead with "swarm" not "Sonnet"; Wildlife Sentinel should lead with "Refiner/Evaluator loop" not the model).

- **Anthropic-aspiration honesty exchange.** User said: "I'd love to work for Anthropic but I don't think Anthropic wants me. I'm not good enough for them." Addressed honestly without dodging: Anthropic specifically hires for ML/RL research backgrounds, safety/alignment papers, and FAANG-style engineering interviews; the user's 16-month career-switch profile is a stretch for that specific hiring bar — not because the portfolio is weak (it isn't) but because Anthropic's selection function is narrow and different. The 100+ companies in the user's actual TAM who *use* Claude as customers of the Anthropic API are a completely different population, and for that population the portfolio reads as genuinely strong. Strategic conclusion: optimize the blog series for the broad population, not for the one team most likely to pass on the user. Anthropic can be a 3-year goal after shipping at a customer company first.

- **Vibe-mode query selection ran into the §10 honesty problem in real time.** The originally-suggested test query "the optimism of the New Deal" triggered the Red Button (similarity score below 0.20), which is correct product behavior — abstract emotional queries don't map cleanly to poster metadata even at 5,000 posters. Bad for the screenshot, though: the hero shot can't include the "I'm not fully confident" banner without contradicting the lede. Cosmetic-crop workarounds were rejected as dishonest (a recruiter trying the same query would see a different experience). Pivoted to a goldilocks pattern: abstract about *intent*, concrete about *subject matter*. Three queries offered: "Bold colorful propaganda from World War II" (chosen — direct hit on WWII propaganda series), "Vintage astronaut and space exploration art" (backup), "Encouraging Americans to save during the war" (backup).

- **Stills → screencast pivot.** Originally planned 2 still images (vibe hero + Archivist detail). Mid-shoot the user hit two real problems with stills: the masonry grid is only 4 cards wide so initial-viewport screenshots only show 3–4 posters; and The Archivist's streaming response is a motion-native feature that loses its punch in a still. User suggested screencast — agreed immediately. Same precedent as ChronoQuizzr, same algorithmic-lift logic, same series-cohesion logic.

- **Video produced: `Recording 2026-05-11 103347.mp4` (1:48, 62 MB).** Frame-by-frame review via `ffmpeg` extraction at 0s, 5s, 15s, 30s, 50s, 70s, 90s, 100s, 107s. Structure: dark-mode landing → theme switch to light → vibe-mode query → result grid → poster detail page → Visually Similar pgvector siblings → Archivist streaming response with grounded entities (Lawson Wood, Bowman Pump Company, WPA Federal Art Project). Climax frame is at ~100s where the Archivist response with named historical entities is fully on screen. End frame at 107s is the strongest single-frame summary of the post's claim.

- **Length decision: 1:48 kept despite being longer than the LinkedIn engagement sweet spot.** User initially apologetic about the length ("I know it's too long..."); pushed back: every section delivers a distinct architectural piece (theme switch = UX polish, mode selector = architecture, vibe query = lede, result grid = catalog depth, Visually Similar = pgvector, Archivist = RAG done right). Audience is recruiters and engineers searching technical hashtags, not entertainment-content consumers. A 1:48 video with 40% completion rate is a stronger algorithmic signal than a 0:42 video with 30%. Length acceptable for this specific post.

- **Sitting on the post.** User is deliberately holding the draft for 24–48h to avoid posting both the ChronoQuizzr and Poster Pilot on the same day. Target publish: Tuesday morning 2026-05-12 or Wednesday morning 2026-05-13 (8–10 AM local). Full draft + video + draft-notes archived at `prior-project-blog-posts/POSTER_PILOT_BLOG.md`.

#### D. Memory files created this session

- `project_linkedin_privacy_audit_2026_05_11.md` — the audit findings, changes made, the deferred decision, and the conflation-to-avoid note.
- `feedback_dont_lead_with_claude.md` — the rule, the reason, and how to apply it to remaining series posts.

#### E. Side note worth recording
